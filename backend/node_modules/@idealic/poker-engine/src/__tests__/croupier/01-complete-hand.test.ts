...
/**
 * This test represents the most basic poker hand scenario where all players just call/check through all streets.
 * It's interesting because it:
 * 1. Tests the fundamental flow of a poker hand from preflop to showdown
 * 2. Verifies correct blind posting and pot calculation
 * 3. Ensures proper street progression (preflop -> flop -> turn -> river)
 * 4. Validates showdown mechanics when all players reach the river
 */
describe('Croupier - Complete Hand', () => {
  it('should complete a hand with all streets checked down', async () => {
    const hand: Hand = {
      variant: 'NT',
      players: ['p1', 'p2', 'p3'],
    ...
    expect(game.isBettingComplete).toBe(true);
    expect(game.players.map(p => p.stack)).toEqual([980, 1040, 980]);
    expect(game.pot).toBe(60); // All streets checked through
